...
from tables import Batting, BattingTotal, Pitching, PitchingTotal
...
class GenerateStatsData(object):
    ...
    def gen_records(self, base_model, record_model, key_lists):
        ...
        for row in self.session.query(base_model)\
                .order_by(base_model.playerID.asc(), base_model.yearID.asc(), base_model.stint.asc()).all():
    ...
    def _create_record(self, stats_class, base_model, base_model_keys, playerID, yearID, stint=1):
        '''
        Create Stats Record
